...
function filterNoPurchaseMember() {
    purchaseRecords.forEach((item) => {
        buyCourseMember.push(item.name);
    })
    let neverBuyMember = members.filter((member) => {
        return buyCourseMember.includes(member) === false;
    })
    console.log(`未購買課程的會員有：${neverBuyMember}\n`);
}
